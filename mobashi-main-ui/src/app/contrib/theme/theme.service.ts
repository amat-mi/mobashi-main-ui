...
export const provideThemeService = (options?: ThemeServiceOptions | Promise<any>) => {
  return {
    ...
    useFactory: (
      ...
      tr: TranslocoService,
    ) => Promise.resolve(options).then((options) => new ThemeService(appConfig, document, storage, tr, options)),
    deps: [
      APP_CONFIG,
      DOCUMENT,
      StorageService,
